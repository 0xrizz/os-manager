...
2. **Restore Konfigurasi WSL:**
   ...
   ```
3. **Pembersihan Partisi Transisi & Reklamasi Ruang Root 235 GB (Zero-USB):**
   * Jalankan otomasi Zero-USB:
     ```bash
...
## 5. Siklus Hidup Lanjutan: Distribusi Upgrade ke Debian 13 (Trixie)

Setelah sistem bare-metal berjalan stabil di Debian 12 (Bookworm) dan seluruh partisi ruang telah direklamasi, migrasi *in-place* ke Debian 13 (Trixie) dikelola secara independen melalui modul `osm upgrade`.

Dokumentasi arsitektur, SRE invariants, mitigasi hardware Intel Ice Lake (Sound Open Firmware, DRM nodes, NetworkManager), dan panduan pemulihan darurat didokumentasikan secara lengkap dalam artifact independen:
👉 **[`docs/DEBIAN_13_TRIXIE_UPGRADE_GUIDE.md`](DEBIAN_13_TRIXIE_UPGRADE_GUIDE.md)**

### Ringkasan Perintah CLI:
```bash
# 1. Audit kesiapan & pre-flight health check
osm upgrade check

# 2. Simulasi upgrade (dry-run)
osm upgrade dry-run

# 3. Eksekusi upgrade live (otomatis membungkus sesi tmux 'osm-trixie-upgrade' & sleep-inhibit)
sudo osm upgrade start

# 4. Verifikasi hardware, DRM & jaringan pasca-reboot
osm upgrade verify

# 5. Rebuild python venv pasca upgrade runtime host
